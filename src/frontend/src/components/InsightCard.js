    ...
    >
      <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column', p: 2, '&:last-child': { pb: 2 } }}>
        ...
        {insights && insights.length > 0 && (
          <>
            <Typography variant="subtitle2" sx={{ mb: 1, mt: 1, fontWeight: 600, fontSize: '0.875rem' }}>
              Top observations
            </Typography>
                  ...
                  <ListItemText
                    primary={
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 1 }}>
                        <Typography variant="body2" sx={{ fontSize: '0.75rem', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {insight.name}
                        </Typography>
                        <Chip
                          label={insight.count}
